...
class Renderer:
    """
    A base class for a series of classes that can render a MIME part into a
    variety of commonly-used output formats (eg. HTML snippets, plain text) by
    specifying a desired target MIME type.
    """
    @classmethod
    def render(cls, part, to_type, inline=False):
        """
        Attempt to render MIMEPart <part> into MIME type <to_type> (a string).
        If <inline> is true, the part will be rendered as part of a document
        and the rendering should be suitable for in-line display. Otherwise
        the MIMEPart is being displayed stand-alone.

        May throw an exception of the rendering cannot be completed or there
        is no suitable handler for the MIME part installed.
        """
